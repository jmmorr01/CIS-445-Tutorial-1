...
if (_Y = .) then do;
   ...
   R_TARGET_B0 = - _P1;
   select( _Y );
      when (0)  R_TARGET_B1 = R_TARGET_B1 + 1;
      when (1)  R_TARGET_B0 = R_TARGET_B0 + 1;
   end;
end;
...
CP_TARGET_B = REGdema [_tarnum,_decnum];

*** Best Possible Consequence of Any Decision without Cost;
array REGdebe [2] _temporary_ ( 14.5 0);
BP_TARGET_B = REGdebe [_tarnum];
